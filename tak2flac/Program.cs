...
public class Program
{
    ...
    public static Metadata GetMediaMetadata(string file)
    { 
        ...
        if (Verbose)
			...
			Metadata result = JsonSerializer.Deserialize<Metadata>(probeData, Utils.JsonOptions);
			return result;
		}
    ...
    static string GetEnv(string envName)
    {
        ...
        if (env is null)
        {
				if (envName == "FFMPEG_BINARY")
				{
					env = Path.Combine(Environment.CurrentDirectory, "ffmpeg.exe");
				}
				else if (envName == "FFPROBE_BINARY")
				{
					env = Path.Combine(Environment.CurrentDirectory, "ffprobe.exe");
				}
				if (!File.Exists(env))
				{
